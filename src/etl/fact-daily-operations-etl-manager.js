    ...
    transform(data) {
        var result = data.map((item) => {
            ...
            return {
                ...
                orderType: item.kanban.productionOrder.orderType ? `'${item.kanban.productionOrder.orderType.name}'` : null,
                ...
                machineCode: item.machine.code ? `'${item.machine.code}'` : null,
                machineCondition: item.machine.condition ? `'${item.machine.condition}'` : null,
                machineManufacture: item.machine.manufacture ? `'${item.machine.manufacture}'` : null,
                machineMonthlyCapacity: item.machine.monthlyCapacity && item.kanban.selectedProductionOrderDetail.uom ? `${this.orderQuantityConvertion(orderUom, item.machine.monthlyCapacity)}` : null,
                machineName: item.machine.name ? `'${item.machine.name}'` : null,
                machineProcess: item.machine.process ? `'${item.machine.process}'` : null,
                machineYear: item.machine.year ? `'${item.machine.year}'` : null,
