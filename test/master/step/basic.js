...
before("#00. connect db", function(done) {
    helper.getDb()
        .then((db) => {
            instanceManager = new StepManager(db, {
                username: "unit-test"
            });
            done();
        })
...
it("#01. should error when create new Step with empty data", function(done) {
    instanceManager.create({})
        ...
        .catch((e) => {
            try {
                e.errors.should.have.property("process");
                e.errors.should.have.property("itemMonitoring");
                done();
            }
            catch (ex) {
                done(e);
...
it("#04. should error when create new data with same process", function(done) {
    var data = Object.assign({}, createdData);
    delete data._id;

    instanceManager.create(data)
        .then((id) => {
            done("Should not be able to create data with same process");
        })
        .catch((e) => {
            try {
                e.errors.should.have.property("process");
...
it("#07. should error when update new data with same process", function(done) {
    var newDataId;
    Step.getNewData()
        ...
        .then((newData) => {
            ...
            newData.process = createdData.process;
            return instanceManager.update(newData);
        })
        .then((id) => {
            done("Should not be able to update data with same process");
        })
        .catch((e) => {
            try {
                e.errors.should.have.property("process");
